...
def test_run_simulation_wiring(mock_pocomc, mock_likelihood, mock_priors, mock_wg, mock_network, mock_nr, tmp_path):
    ...
    # Verifications
    # 1. NR data loaded (conceptually, even if analytic used for config, spine checks waveform prop?
    # No, it uses nr_data for metadata)
    # Actually, if injection mode is 'analytic', spine.py still might try to load NR data
    # if sim_config.waveform was passed?
    # In the new logic:
