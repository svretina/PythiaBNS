...
class VSB_R_Relation(KoutaliosRelation):
    def predict(self, m1: float, m2: float, eos_name: str) -> dict[str, float]:
        ...
        fpeak = (
            Mchirp
            * (10.942 - 0.369 * Mchirp - 0.987 * R18 + 1.095 * Mchirp**2 - 0.201 * Mchirp * R18 + 0.036 * R18**2)
            * 1e3
        )
        f20 = (
            Mchirp
            * (8.007 + 4.356 * Mchirp - 1.241 * R18 + 0.558 * Mchirp**2 - 0.375 * Mchirp * R18 + 0.054 * R18**2)
            * 1e3
        )
        fspiral = (
            Mchirp
            * (5.846 + 1.75 * Mchirp - 0.555 * R18 + 1.002 * Mchirp**2 - 0.316 * Mchirp * R18 + 0.026 * R18**2)
            * 1e3
        )

        return {"f_peak": fpeak, "f_20": f20, "f_spiral": fspiral}
